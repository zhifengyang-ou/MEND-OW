        ...
        do i = 1, sINI%nVARopt
            ...
            if(sINI % iModel.eq.4) then !! MCMC
                ...
                if(trim(sINI%VARobj(j)).eq."NSEC") then
                    sINI%rOBJ(i) = f1NSE(sINI%VARopt_int(i,2), sINI%dOBS_opt(ibeg:iend,2), &
                        sINI%dSIM_opt(ibeg:iend,2), const_FillValue)
                else if (trim(sINI%VARobj(j)).eq."MARE") then !! MARE
                    sINI%rOBJ(i) = sINI%VARobj_tol(j)*fMARE(sINI%VARopt_int(i,2),sINI%dOBS_opt(ibeg:iend,2),&
                        sINI%dSIM_opt(ibeg:iend,2),const_FillValue) + &
                        (1.d0-sINI%VARobj_tol(j))* f1RAVG_ratio(sINI%VARopt_int(i,2),sINI%dOBS_opt(ibeg:iend,2),&
                        sINI%dSIM_opt(ibeg:iend,2), 1.d0, const_FillValue)
            ...
            end if !! (sINI % iModel.eq.4) then !! MCMC
        end do !!i = 1, sINI%nVARopt
...
!                    write(*,*)iday,k,sINI%SIN_NH4(k),sINI%SIN_NO3(k),sOUT%MNFLUX%NH4_dep,sOUT%MNFLUX%NO3_dep
                ...
                !!---------------------------------------------
                    ...
                    if(j==0) j=sINI%nHour
                    !!sINP%GPP = SIN_mean(j) zhifeng, the SIN_mean is used in 1-year mean hourly data
                    sINP%GPP = sINI%SIN(j)
...
!! unit of NOxAir: g N m-3
    ...
    END SELECT
        ...
        fNOxAir = NOxAir_ppb * 1e-9 * VolAir / (const_R * (Temperature + const_tmp_C2K)) * MolMass * nN *  Pressure 
...
!    pressure = fPa(Altitude) !!kPa
    Gas_Conc_Air = fNOxAir(Ngas, Altitude, Temperature)*1d3/1d6  !!g N/m3 = mg N/cm3
...
!                dSIM(iHour,j) = sOUT%CPOOLI(2)%SOM
            ...
            case (29)   !!Enzyme concentration for nitrogen fixation
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(1)
            case (30)   !!Enzyme concentration for nitrification
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(2)
            case (31)   !!Enzyme concentration for denitrification NO3 - NO2
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(3)
            case (32)   !!Enzyme concentration for denitrification NO2 - NO
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(4)
            case (33)   !!Enzyme concentration for denitrification NO - N2O
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(5)
            case (34)   !!Enzyme concentration for denitrification N2O - N2
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(6)
            case (35)   !!ENZ activity for nitrogen fixation
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(1)*sPAR%Vnif
            case (36)   !!ENZ activity for nitrification
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(2)*sPAR%Vnit                
            case (37)   !!ENZ activity for denitrification NO3-NO2
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(3)*sPAR%Vdenit(1)               
            case (38)   !!ENZ activity for denitrification NO2-NO
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(4)*sPAR%Vdenit(2)       
            case (39)   !!ENZ activity for denitrification NO-N2O
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(5)*sPAR%Vdenit(3)       
            case (40)   !!ENZ activity for denitrification N2O-N2
                dSIM(iHour,j) = sOUT%CPOOL%ENZNm(6)*sPAR%Vdenit(4)                            
            case default
                dSIM(iHour,j) = sOUT%MNFLUX%CO2_gmo
        end select
    end do
